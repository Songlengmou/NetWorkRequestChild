package com.anningtex.networkrequestchild.demo.utils
...
import com.anningtex.networkrequestchild.demo.weight.LoadingCallback
...
object SettingUtil {
    ...
    fun getColorStateList(color: Int): ColorStateList {
        val colors =
            intArrayOf(color, ContextCompat.getColor(MyApplication.mContext, R.color.colorGray))
        val states = arrayOfNulls<IntArray>(2)
        states[0] = intArrayOf(android.R.attr.state_checked, android.R.attr.state_checked)
        states[1] = intArrayOf()
        return ColorStateList(states, colors)
